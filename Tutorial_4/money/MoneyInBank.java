public class MoneyInBank implements MoneyTransaction{
    ...
    /*
     * Inisialisasi amount sesuai masukan dan interest rate sebesar interestRate% dan totalLoanAmount sesuai masukan
     */
    ...
    public MoneyTransaction subtract(MoneyTransaction money) {
        if (money.getAmount() > this.amount){
            MoneyTransaction a = new MoneyInBank(0, this.interestRate, this.totalLoanAmount);
            return a;
        }
        else{
            MoneyTransaction a = new MoneyInBank(this.amount-money.getAmount(), this.interestRate, this.totalLoanAmount);
            return a;
        }
    }
    ...
    public void borrow(int loanAmount) {
        amount += loanAmount;
        totalLoanAmount += ((loanAmount * (100 + interestRate)) / 100);
    }
